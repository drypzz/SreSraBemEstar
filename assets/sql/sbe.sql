...
DROP TABLE IF EXISTS `cadresponsavel`;
CREATE TABLE `cadresponsavel` (
...
DROP TABLE IF EXISTS `cod_idoso_remed`;
CREATE TABLE `cod_idoso_remed` (
...
DROP TABLE IF EXISTS `idoso_como`;
CREATE TABLE `idoso_como` (
...
ALTER TABLE `cadidoso`
  ADD CONSTRAINT `CPF_Resp` FOREIGN KEY (`CPF_Resp`) REFERENCES `cadresponsavel` (`CPF_Resp`);
...
ALTER TABLE `comorbidade`
  ADD CONSTRAINT `Cod_Nivel` FOREIGN KEY (`Cod_Nivel`) REFERENCES `nivel` (`Cod_Nivel`);

ALTER TABLE `idoso_como`
  ADD CONSTRAINT `Cod_Como` FOREIGN KEY (`Cod_Como`) REFERENCES `comorbidade` (`Cod_Como`),
  ADD CONSTRAINT `idoso_como_ibfk_1` FOREIGN KEY (`CPF_Idoso`) REFERENCES `cadidoso` (`CPF_Idoso`) ON DELETE SET NULL ON UPDATE SET NULL;

ALTER TABLE `tarefa`
  ADD CONSTRAINT `Cod_Agen` FOREIGN KEY (`Cod_Agen`) REFERENCES `agenda` (`Cod_Agen`),
  ADD CONSTRAINT `tarefa_ibfk_1` FOREIGN KEY (`Cod_Remedio`) REFERENCES `remedio` (`Cod_Remedio`) ON DELETE SET NULL ON UPDATE SET NULL;
COMMIT;